...
import { MessageCircleIcon, HeartIcon, ThumbsUpIcon, Share2Icon } from "lucide-react"
...
const Post = (props: ExtendedProps) => {
  ...
  function likePost() {
    if (!props.currentUserId) {
      alert("Log in to like posts") // TODO
    }
  }

  return (
    <>
      <section className="flex flex-col shadow-md gap-1 rounded-md bg-[--off-bg-main] w-[450px] lg:w-[500px]">
        {hidePost ? (
          ...
          <>
            <div className="flex pt-3 px-3 flex-col gap-2">
              <div className="flex justify-between items-center">
                <div className="flex gap-3">
                  <div className="flex items-center">
                    <Link href={`/${props.author.profileLink}`} className={`w-10 h-10 rounded-full overflow-hidden flex justify-center items-center`}>
                      <Image src={`/${props.author.profileImage}`} className={`object-cover w-10 h-10`} alt="profile-image" width={48} height={48} />
                    </Link>
                  </div>
                  <div className="flex flex-col justify-start">
                    <Link href={`/${props.group?.profileLink || props.author.profileLink}`}>
                      <p className="font-semibold hover:underline">{props.group?.name || props.author.name}</p>
                    </Link>
                    <div className="flex items-center text-sm text-[--off-text-main]">
                      ...
                      {props.audience === Audience.PUBLIC && (
                ...
                <div className="flex justify-center items-center gap-1 relative">
                  <div
                    ...
                    className="flex justify-center items-center rounded-full cursor-pointer hover:bg-[--off-bg-main-off] w-8 h-8 p-1"
                  >
              ...
              <div onClick={() => setExpandPost(true)}>
                ...
                <div className="flex gap-2 items-center">
                  {props.comments.length > 0 && (
                    <div className="flex items-center">
                      ...
                      <MessageCircleIcon className="w-4 h-4 fill-[--off-text-main] border-none" />
                    </div>
                  )}
                  {props.shares.length > 0 && (
                    <div className="flex items-center">
                      <p>{`${props.shares.length}`}</p>
                      <Share2Icon className="w-4 h-4 fill-[--off-text-main] border-none" />
                    </div>
                  )}
                </div>
              </div>
              <div className="flex items-center justify-between border-t border-[--off-bg-main-off-fade] py-1 px-6">
                <LikeButton likes={props.likes} postId={props.id} setPostLikes={setPostLikes} />
                <div onClick={() => setExpandPost(true)} className="py-1.5 px-6 rounded-md flex items-center justify-center gap-2 cursor-pointer hover:bg-[--off-bg-main-off]">
                  <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24" className="fill-[--text-main]">
                  ...
                  <p className="text-[--text-main] font-medium">Comment</p>
                </div>
                <div onClick={() => setExpandPost(true)} className="py-1.5 px-6 rounded-md flex items-center justify-center gap-2 cursor-pointer hover:bg-[--off-bg-main-off]">
                  <svg xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 -960 960 960" width="24" className="fill-[--text-main]">
                  ...
                  <p className="text-[--text-main] font-medium">Share</p>
                </div>
              </div>
              {/* Comment Snippet */}
              {props.comments.length > 0 && <CommentSnippet postComments={props.comments} setExpandPost={setExpandPost} />}
            </div>
          </>
        )}
      </section>
